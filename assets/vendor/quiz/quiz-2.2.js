...
function getQuizAnswers(postId) {
    ...
    $.ajax({
        type: 'POST',
        url: TrConfig.baseURL + '/AjaxController/getQuizAnswers',
...
function getQuizResults(postId) {
    ...
    $.ajax({
        type: "POST",
        url: TrConfig.baseURL + '/AjaxController/getQuizResults',
        data: setAjaxData(data),
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
...
function showTriviaQuizResult() {
    if (arrayQuizResults.length > 0) {
        ...
        if (result.length > 0) {
            result = '<p class="quiz-score"><span class="correct">' + TrConfig.textCorrectAnswer + ': <b>' + quizCorrectCount + '</b></span> <span class="wrong">' + TrConfig.textWrongAnswer + ': <b>' + (quizAnsweredQuestions.length - quizCorrectCount) + '</b></span></p>' + result;
...
$(document).on('click', '.quiz-question .answer_poll', function () {
    ...
    if (TrConfig.authCheck != 1 && isPublic == 0) {
    ...
    $.ajax({
        type: 'POST',
        url: TrConfig.baseURL + '/AjaxController/addPostPollVote',
        data: setAjaxData(data),
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
